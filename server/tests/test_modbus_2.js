...
const logStatus = (mb) => {
  ...
  process.stdout.write(
    ...
    ` | GRID: ${mb.grid.toFixed(0).padStart(6)}W` +
    ` | LOAD: ${load.toFixed(0).padStart(6)}W` +
    ` | SOC: ${mb.soc}% `
  );
};
...
async function runComparisonTest() {
  ...
  try {
    ...
    const TARGET_WATTS   = -4000;
    const RAW_POWER      = Math.round(32000 + TARGET_WATTS); // 28000 = charge
    const TARGET_SOC_PCT = 80;
