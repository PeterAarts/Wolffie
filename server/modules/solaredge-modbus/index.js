...
class SolarEdgeModule {
  ...
  /**
   * Re-reads settings from DB and re-injects into collector and api.
   * Called by the core settings route after any setting change.
   * Fixed: api was referenced in original but never imported — added above.
   */
  async reinitialize() {
   ...
   * an extra Modbus round-trip on every capability request.
   *
   * solar:read  priority 10 — SolarEdge is the authoritative solar source.
   * grid:read   priority 6  — fallback; a P1 meter wins at priority 10.
   * solar:curtail priority 10 — stop/resume production via power limit register.
   */
  _registerCapabilities() {
    ...
    console.log(`     - Capabilities registered: solar:read (10), solar:curtail (10), grid:read (6)`);
  }
}
