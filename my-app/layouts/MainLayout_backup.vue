...
watch(
  ...
  async (newPath, oldPath) => {
    // 1. Only run if we are moving BETWEEN settings pages
    // 2. Ignore the very first load (where oldPath is undefined/empty)
    if (newPath.includes('/settings') && oldPath && newPath !== oldPath) {
      console.log('Syncing state for navigation to:', newPath);
      try {
