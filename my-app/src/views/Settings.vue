  ...
  <div class="flex flex-col md:flex-row  text-gray-900">
    ...
    <!--  <div class="p-6 border-b border-gray-100">
      ...
      <nav class="flex-1 overflow-y-auto p-4 pe-0 space-y-4 settingsmenu">
        ...
        <button v-for="item in staticMenu" :key="item.id"
          ...
          class="w-full flex items-center p-3 transition-all duration-200 group text-sm"
   ...
   <!--       <i :class="[item.icon, 'w-6 text-lg', activeModuleId === item.id ? 'text-white' : 'text-gray-500 group-hover:text-gray-900']"></i>-->
        ...
        <button v-for="mod in settingsModules" :key="mod.module_id"
          ...
          class="w-full flex items-center p-2 transition-all duration-200 group text-sm text-gray-700"
          :class="[
...
const staticMenu = [
  { id: 'core', label: t('settings.general'), icon: 'fa-duotone fa-server' },
  { id: 'users', label: t('settings.usermanagement'), icon: 'fa-duotone fa-users-gear' },
];
