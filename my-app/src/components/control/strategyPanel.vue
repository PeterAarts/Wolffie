...
<template>
  <div class="strategy-panel">
    ...
    <div v-if="activeTab === 'strategy'" class="strategy-tab">
        ...
        >
          ...
          <div class="flex items-start justify-between w-full">
            ...
            <div class="flex flex-col items-end gap-1 shrink-0 ml-2">
              ...
              <i class="fa-light text-secondary-400 text-xs mt-1"
                :class="expandedStrategy === s.id ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
            </div>
          </div>
        </button>
      </div>

      <!-- Expanded strategy panel — slides in below cards -->
      <transition
      ...
      >
        <div v-if="expandedStrategy" class="strategy-expand-panel">

          <!-- Header with activate button -->
          <div class="expand-panel__header">

            <button
              v-if="expandedStrategy !== strategyStore.activeId"
              class="btn btn--primary btn--sm"
              :disabled="strategyStore.isLoading"
              @click="selectStrategy(expandedStrategy)"
            >
              <i class="fa-light fa-check mr-1.5"></i>
              {{ t('control.activate') }}
            </button>
            <span v-else class="text-xs text-green-600 font-semibold flex items-center gap-1">
              <i class="fa-light fa-circle-check"></i>
              {{ t('control.currentlyActive') }}
            </span>
          </div>

          <!-- Config fields — only for smart-eco, others show capability list -->
          <div v-if="expandedStrategy === 'smart-eco'" class="expand-panel__config">
            ...
            <div v-if="nightlyProfile" class="profile-readout">
          ...
          <!-- For other strategies: show required/optional capabilities -->
          <div v-else class="expand-panel__caps">
            <div v-if="availableStrategies.find(s=>s.id===expandedStrategy)?.requiredCapabilities?.length" class="caps-group">
              <span class="caps-group__label">{{ t('control.requires') }}</span>
              <span v-for="cap in availableStrategies.find(s=>s.id===expandedStrategy).requiredCapabilities"
                :key="cap" class="cap-badge cap-badge--required">
                <i class="fa-light fa-circle-check mr-1"></i>{{ cap }}
              </span>
            </div>
            <div v-if="availableStrategies.find(s=>s.id===expandedStrategy)?.optionalCapabilities?.length" class="caps-group">
              <span class="caps-group__label">{{ t('control.optional') }}</span>
              <span v-for="cap in availableStrategies.find(s=>s.id===expandedStrategy).optionalCapabilities"
                :key="cap"
                class="cap-badge"
                :class="availableStrategies.find(s=>s.id===expandedStrategy).activeOptional?.includes(cap)
      ...
      <div class="quick-actions">
        ...
        <div class="quick-actions__row">
          <button class="btn btn--sm" :class="{ 'btn--busy': acting === 'normal' }"
            :disabled="!!acting" @click="act('normal')">
            {{ t('control.normal') }}
          </button>
          <button class="btn btn--sm" :class="{ 'btn--busy': acting === 'prevent' }"
            :disabled="!!acting" @click="act('prevent')">
            {{ t('control.preventDischarge') }}
          </button>
          <button class="btn btn--sm btn--destructive" :class="{ 'btn--busy': acting === 'stop' }"
            :disabled="!!acting" @click="act('stop')">
            {{ t('control.stopDispatch') }}
          </button>
        </div>
      </div>

      <!-- ── 4. DAY-AHEAD CHART (collapsible) ───────────────────────────────── -->
      <div class="validate-section">
        <button class="validate-toggle" @click="showChart = !showChart">
          <i class="fa-light fa-chart-mixed mr-1.5"></i>
          {{ t('control.validateStrategy') }}
          <i class="fa-light ml-auto" :class="showChart ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
        </button>
        <transition
          enter-active-class="transition-all duration-200 ease-out"
...
function toggleStrategyExpand(id) {
  if (!availableStrategies.value.find(s => s.id === id)?.available) return;
...
async function act(action) {
  const map = {
    normal:  { url: '/capability/battery/stop',             msg: t('control.normalRestored') },
    prevent: { url: '/capability/battery/prevent-discharge', msg: t('control.dischargeBlocked') },
    stop:    { url: '/capability/battery/stop',             msg: t('control.dispatchStopped') },
  };
  acting.value = action;
  try {
...
.strategy-card--selected              
                        { border-color: var(--color-secondary-300);background-color: var(--color-secondary-50);margin-bottom: calc(-1.5rem - 1px) ;border-bottom:1px solid var(--color-bg-secondary-50);border-bottom-left-radius: 0px;border-bottom-right-radius: 0px;}
...
.strategy-card--selected .strategy-card__top-bar { background: var(--color-secondary-100); }
...
/* ── Expand panel ──────────────────────────────────────────────────────── */
.strategy-expand-panel  { border: 1px solid var(--color-secondary-300);background: var(--color-secondary-50);padding: 1.25rem;display: flex; flex-direction: column; gap: 1rem;}
.expand-panel__header   { display: flex; align-items: flex-start;justify-content: space-between; gap: 1rem;}
.expand-panel__config   { display: flex; flex-direction: column; gap: 0.875rem; }

/* Capabilities */
.expand-panel__caps     { display: flex; flex-direction: column; gap: 0.625rem; }
...
.validate-section       { border-top: 1px solid var(--color-secondary-200); padding-top: 0.875rem; }
.validate-toggle        { display: flex; align-items: center; gap: 0.5rem; width: 100%;background: none; border: none; cursor: pointer;font-size: 0.8125rem; font-weight: 500;color: var(--color-text-secondary);padding: 0.25rem 0;transition: color 0.12s;}
.validate-toggle:hover  { color: var(--color-text-primary); }
