...
async function fetchSparklineData(socket) {
  const uiId  = socket.id;                              // UI key (46809)
  const apiId = socket.device_settings_id ?? socket.id; // API key (2)

  sparklineLoading.value = { ...sparklineLoading.value, [uiId]: true };
  sparklineError.value   = { ...sparklineError.value,   [uiId]: false };

  try {
    const response = await apiClient.get(`/${socket.source}/devices/${apiId}`);
    const raw = response?.data?.data || [];

    sparklineData.value = {
      ...sparklineData.value,
      [uiId]: Array.isArray(raw)
        ? raw.filter(d => d?.timestamp && d?.power != null)
             .map(d => ({ timestamp: d.timestamp, power: parseFloat(d.power) || 0 }))
        : []
    };
  } catch {
    sparklineError.value = { ...sparklineError.value, [uiId]: true };
    sparklineData.value  = { ...sparklineData.value,  [uiId]: [] };
  } finally {
    sparklineLoading.value = { ...sparklineLoading.value, [uiId]: false };
  }
}
// ── Interaction ───────────────────────────────────────────────────────────────
